...
public partial class adm_drafts : System.Web.UI.Page
{
    SqlCommand cmd;
    SqlDataAdapter da;
    DataSet ds;
    ...
    DataTable GetData()
    {
        ...
        string strcon = "select * from job_site_posts where post_status=@post_status ORDER BY sr DESC";
        cmd = new SqlCommand(strcon, con);
        cmd.Parameters.AddWithValue("@post_status", "Draft");
    ...
    protected void publishselectedbtn_Click(object sender, EventArgs e)
    {
        sucesspanel.Visible = false;
        errorpanel.Visible = false;
        foreach (RepeaterItem Item in Repeater1.Items)
        {
            ...
            if (chkDelete.Checked)
            {
                ...
                DataTable dt = new DataTable();
                SqlConnection con = new SqlConnection(DecryptString(System.Configuration.ConfigurationManager.AppSettings["cn"], EncryptionKey2));
                string strcon = "update job_site_posts Set post_status=@post_status where sr=@sr";
                cmd = new SqlCommand(strcon, con);
                cmd.Parameters.AddWithValue("@sr", hdnID_value.ToString());
                cmd.Parameters.AddWithValue("@post_status", "Published");
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
                con.Dispose();
                sucesslbl.Text = "Post Published Sucessfully!";
            }
        }
        try
        {
